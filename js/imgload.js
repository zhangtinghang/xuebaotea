...
var DynamicAvatar = 0;
var teaDynamicCover = 0;
var StudentAvatar = 0;
var AllStuAvatar = 0;
var classStuArr = 0;
//渲染老师动态头像
function teaDynamicAvatar(obj,arrImg){
	if(arrImg.length !=0){
		var imgUrl = arrImg[DynamicAvatar].teacherImage;
		DynamicAvatar++; 
		load(obj,imgUrl);
	}
}
//老师动态封面
function teaDynamicCover(obj,arrImg){
	if(arrImg.length !=0){
		var imgUrl = arrImg[teaDynamicCover].images;
		teaDynamicCover++; 
		load(obj,imgUrl);
	}
}
...
//学生列表所有头像
function teaAllStuAvatar(obj,arrImg){
	if(arrImg.length !=0){
		var imgUrl = arrImg[AllStuAvatar].student.studentImage;
		AllStuAvatar++; 
		load(obj,imgUrl);
	}
